...
export function FacilityPanel({
  ...
  return (
    <aside
      className={`absolute inset-y-0 right-0 z-20 flex w-[380px] max-w-[90vw] flex-col border-l border-white/10 bg-slate-950/95 shadow-2xl backdrop-blur transition-transform duration-200 ${
    ...
    >
      <header className="flex items-start justify-between gap-2 border-b border-white/10 px-4 py-3">
        <div>
          <h2 className="text-sm font-semibold text-slate-100">Facility receipts</h2>
          <p className="text-xs text-slate-400">
            Evidence behind <span className="text-slate-200">{capabilityLabel}</span> coverage
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex gap-2 border-b border-white/10 px-4 py-3">
        <label className="flex-1 text-xs">
          ...
          <select
            ...
            className="w-full rounded-md border border-white/15 bg-slate-900 px-2 py-1.5 text-sm text-slate-100 outline-none focus:border-blue-400"
          >
        ...
        <label className="flex-1 text-xs">
          ...
          <select
            ...
            className="w-full rounded-md border border-white/15 bg-slate-900 px-2 py-1.5 text-sm text-slate-100 outline-none focus:border-blue-400 disabled:opacity-50"
          >
      ...
      <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
        ...
        {state && loading && <p className="mt-8 text-center text-sm text-slate-500">Loading facilities…</p>}
        {state && !loading && error && <p className="mt-8 text-center text-sm text-rose-400">{error}</p>}
